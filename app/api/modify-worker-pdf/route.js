...
export async function POST(req) {
  try {
    console.log("Processing Worker PDF modification...");
    ...
    const textFields = [
      ...
      { text: ownerName?.trim() || "No Data", x: 75, y: 59 },
    ...
    console.log("🎉 Worker PDF modified successfully!");
    ...
    return new Response(pdfBytes, {
      ...
      headers: {
        ...
        "Content-Disposition": "inline; filename=worker-modified.pdf",
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, OPTIONS",
      },
    });
  } catch (error) {
    console.error("❌ Error modifying Worker PDF:", error);
    return new Response(
      JSON.stringify({
        message: "Error modifying Worker PDF",
        error: error.message,
      }),
